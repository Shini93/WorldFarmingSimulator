...
class RessourceManager {
  ...
  Boolean findField(int k, int i) {
    PVector pos = getRessourcePos(k, i);
    if (mouseX < pos.x - ressSize || mouseX > pos.x + ressSize)
      return false;
    if (mouseY < pos.y - ressSize || mouseY > pos.y + ressSize)
      return false;
    return true;
  }
}
